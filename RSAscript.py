...
def send_file(file, host):
    ...
    ssh.connect(hostname="", username="", password="")  # Enter the recipient's hostname, username, and password

    ftp = ssh.open_sftp()  # Open an SFTP session for secure file transfer
    
    # Upload the encrypted file to the recipient's server
    ftp.put("encrypted.txt", "")  # Specify the destination path for the file on the recipient's server
    ftp.close()
    ssh.close()
